...
sys_decrement_priority(envid_t envid)
{
	...
	if (err == 0 && dstenv->env_parent_id != 0) {
		dstenv->env_prioritry = parentEnve->env_prioritry;
		// cprintf("parentEnve->env_prioritry %d \n",parentEnve->env_prioritry);
		// cprintf("TIENE PADRE: envid %d envid_parent %d \n",envid,dstenv->env_parent_id);
	}

	if (dstenv->env_prioritry > 1) {
		dstenv->env_prioritry--;
		dstenv->env_count_interruptions++;
	}
